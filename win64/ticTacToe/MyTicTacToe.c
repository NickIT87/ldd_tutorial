...
enum { CROSSES, NOUGHTS, BORDER, EMPTY };
...
int GetComputerMove(int *board, const int side) {
	...
	/*printf("Переменные positions:%d maxDepth:%d bestMove:%d\n", positions,maxPly,best);*/
	return best;	
}
...
int GetHumanMove(const int *board) {
	...
	while (moveOk == 0) {
		...
		printf("type move number from 1 to 9:");
		...
		if(move < 1 || move > 9) {
			move = -1;
			printf("your choice not in range from 1 to 9!\n");
			continue;
		}
		...
		if(board[ConvertTo25[move]]!=EMPTY) {
			move=-1;
			printf("This field is busy, try again!\n");
			continue;
		} 
		moveOk = 1;
	}
...
void RunGame() {
	...
	while(!GameOver) {
		...
		if(FindThreeInARow(board, LastMoveMade, Side ^ 1) == 3) {
			...
			GameOver = 1;
			if(Side==CROSSES) {
				printf("You loose!, Computer Wins!\n");
			} else {
				printf("You win!, Human Wins!\n");
			}
		}
